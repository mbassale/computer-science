...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Counting Distinct Keys with Quicksort." << std::endl;
    ...
    std::mt19937 gen(rd());
    std::uniform_int_distribution <size_t> dist(0, numbers_length);
    ...
    std::cout << "Counting distinct keys... " << std::flush;
    size_t distinct_keys = 1;
    for (size_t i = 1; i < numbers.size(); i++) {
        if (numbers[i] != numbers[i - 1]) distinct_keys++;
    }
    std::cout << "Done." << std::endl;
