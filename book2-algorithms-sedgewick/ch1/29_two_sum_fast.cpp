...
size_t two_sum_fast(std::vector<long> &numbers) {
    size_t sum_count = 0;
    // n * log(n) sorting.
    ...
    while (it != numbers.end()) {
        ...
        if (std::binary_search(it, numbers.end(), -num)) {
            sum_count++;
        }
        it++;
    }
...
int main(int argc, char *argv[]) {
    ...
    cout << "Two Sum Problem with Binary Search." << endl;
    ...
    auto sum_count = two_sum_fast(numbers);
    auto t2 = high_resolution_clock::now();
    cout << "Done." << endl;
    auto running_time = duration_cast<milliseconds>(t2 - t1).count();
    cout << "Number of Zero Two Sum: " << sum_count << " Running Time: " << running_time << "ms" << endl;
    return EXIT_SUCCESS;
}
