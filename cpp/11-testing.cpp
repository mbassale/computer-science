...
void sensitivity_greater_then_1() {
    MockServiceBus bus{};
    AutoBrake auto_brake{ bus };
    try {
...
void alert_when_imminent() {
    ...
    assert_that(bus.last_command.time_to_collission_s == 1L, "time to collision not computed correctly.");
}
...
int main() {
    ...
    run_test(sensitivity_greater_then_1, "sensitivity greater than 1");
    run_test(speed_is_saved, "speed is saved");
    run_test(alert_when_imminent, "alert when imminent");
    run_test(no_alert_when_not_imminent, "not alert when not imminent");
    return 0;
}